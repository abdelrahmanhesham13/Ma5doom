...
public class ReservationFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new ReservationAdapter(getContext(), null, true, new ReservationAdapter.OnDataChanged() {
            @Override
    ...
    public void loadData(){
        if (!NetworkUtils.isNetworkConnected(getContext())) {
            GMethods.showSnackBarMessage("تأكد من الاتصال بالانترنت ثم اعد المحاوله",(AppCompatActivity)getActivity());
            return ;
        }
            ...
            Service.getAllReservations(getContext() , isCurrent?"0":"1", new RequestCallBack() {
                ...
                public void success(String response) {
                    ...
                    if (reservation.getStatus()){
                        adapter.changeData((ArrayList<Product>) reservation.getProducts() , isCurrent);
                    } else {
                    ...
                    if (reservation.getProducts()==null || reservation.getProducts().size()<1){
                        empty_view.setVisibility(View.VISIBLE);
                        if (isCurrent){
                            empty_view.setText("لا يوجد حجوزات حاليه");
                        } else {
